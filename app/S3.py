...
Dual-client design:
  - `client`        uses the INTERNAL endpoint (http://minio:9000) for uploads,
                    downloads, deletes — things that happen inside the docker network.
  - `presign_client` uses the PUBLIC endpoint (e.g. http://<ec2-ip>:9000) so the URLs
                    it signs are reachable by the end user's browser.

Both clients sign with the same credentials, so the public presigned URLs are
valid against the real MinIO instance — they just embed a different host.
...
class S3Client:
    def __init__(self):
        # Internal client — used for actual S3 ops inside the docker network
        ...
        # Presign client — used ONLY to generate URLs the browser will hit.
        # Falls back to the internal endpoint if no public one is configured.
        self.presign_client = boto3.client(
            "s3",
            endpoint_url=S3_PUBLIC_ENDPOINT or S3_ENDPOINT,
            aws_access_key_id=S3_ACCESS_KEY,
            aws_secret_access_key=S3_SECRET_KEY,
            region_name=S3_REGION,
            config=BotoConfig(signature_version="s3v4"),
    ...
    def upload_bytes(self, s3_key: str, data: bytes, content_type: str = "application/octet-stream") -> int:
    ...
    def generate_presigned_upload(self, s3_key: str, content_type: str = "application/octet-stream") -> str:
        """Generate a pre-signed URL for uploading directly to S3 (browser-reachable)."""
    ...
    def generate_presigned_download(self, s3_key: str) -> str:
        """Generate a pre-signed URL for downloading from S3 (browser-reachable)."""
        url = self.presign_client.generate_presigned_url(
            "get_object",
            Params={
                "Bucket": self.bucket,
